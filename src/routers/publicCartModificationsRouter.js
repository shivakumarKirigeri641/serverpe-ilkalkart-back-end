 ...
 */
publicCartModificationsRouter.delete("/remove-item-from-cart", async (req, res) => {
  try {
    let { ipAddress, visitTime, devicename, result_ipdetails } = await getRequestDetails(req);    
    let result = validateForItemInCart(req);
    if(true === result.successstatus){
      result = await removeItemFromCart(ipAddress, devicename, req.body.combined_code);
    }
    return res.status(result.statuscode).json({
      statuscode: result.statuscode,
      powered_by: "ServerPe App Solutions",
 ...
 */
publicCartModificationsRouter.put("/reduce-item-from-cart", async (req, res) => {
  try {
    let { ipAddress, visitTime, devicename, result_ipdetails } = await getRequestDetails(req);    
    let result = validateForItemInCart(req);
    if(true === result.successstatus){
